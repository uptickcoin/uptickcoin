  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/uptickcoin/uptickcoin "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/uptickcoind src/uptickcoin-cli src/uptickcoin-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/uptickcoin-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/uptickcoin-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/uptickcoin-core-${tag:1}"
        fi
        URL="https://uptickcoin.org/$BIN_PATH/uptickcoin-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "uptickcoin-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "uptickcoin-${tag:1}"
        rm "uptickcoin-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
