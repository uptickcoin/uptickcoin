...
# Determine output paths to use in UTCS_* environment variables
UTCS_GLIBC="$(store_path glibc-cross-${HOST})"
UTCS_GLIBC_STATIC="$(store_path glibc-cross-${HOST} static)"
UTCS_KERNEL="$(store_path linux-libre-headers-cross-${HOST})"
UTCS_GCC="$(store_path gcc-cross-${HOST})"
UTCS_GCC_LIBS=( "${UTCS_GCC}/lib/gcc/${HOST}"/* ) # This expands to an array of directories...
UTCS_GCC_LIB="${UTCS_GCC_LIBS[0]}" # ...we just want the first one (there should only be one)

# Set environment variables to point Guix's cross-toolchain to the right
# includes/libs for $HOST
#
# NOTE: UTCS_C_INCLUDE_PATH is missing ${UTCS_GCC_LIB}/include-fixed, because
# the limits.h in it is missing a '#include_next <limits.h>'
#
export UTCS_C_INCLUDE_PATH="${UTCS_GCC_LIB}/include:${UTCS_GLIBC}/include:${UTCS_KERNEL}/include"
export UTCS_CPLUS_INCLUDE_PATH="${UTCS_GCC}/include/c++:${UTCS_GCC}/include/c++/${HOST}:${UTCS_GCC}/include/c++/backward:${UTCS_C_INCLUDE_PATH}"
export UTCS_LIBRARY_PATH="${UTCS_GCC}/lib:${UTCS_GCC}/${HOST}/lib:${UTCS_GCC_LIB}:${UTCS_GLIBC}/lib:${UTCS_GLIBC_STATIC}/lib"

# Sanity check UTCS_*_PATH directories
IFS=':' read -ra PATHS <<< "${UTCS_C_INCLUDE_PATH}:${UTCS_CPLUS_INCLUDE_PATH}:${UTCS_LIBRARY_PATH}"
for p in "${PATHS[@]}"; do
    if [ ! -d "$p" ]; then
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform uptickcoin-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'uptickcoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'uptickcoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'uptickcoin-*.tar.gz')"
# Determine our distribution name (e.g. uptickcoin-0.18.0)
