...
###   uptickcoincore.org and uptickcoin.org and compares them.
...
WORKINGDIR="/tmp/uptickcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://uptickcoincore.org"
HOST2="https://uptickcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="uptickcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'uptickcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "uptickcoin.org failed to provide signature file, but uptickcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "uptickcoin.org and uptickcoincore.org signature files were not equal?"
