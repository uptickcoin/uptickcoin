...
The headless daemon `uptickcoind` has the JSON-RPC API enabled by default, the GUI
`uptickcoin-qt` has it disabled by default. This can be changed with the `-server`
...
- **Securing remote network access:** You may optionally allow other
  ...
  `uptickcoind -help` for more information about these settings and other
