Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://uptickcoincore.org/bin/uptickcoin-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/uptickcoin/uptickcoin/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `uptickcoind -help` output for these two options for
...
- The `testnet` field in `uptickcoin-cli -getinfo` has been renamed to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 UpTickCoin address without
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (uptickcoinVBR)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/uptickcoin/uptickcoin/).
