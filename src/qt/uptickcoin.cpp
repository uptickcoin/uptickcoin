...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. uptickcoin_de.qm (shortcut "de" needs to be defined in uptickcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. uptickcoin_de_DE.qm (shortcut "de_DE" needs to be defined in uptickcoin.qrc)
...
static const char* qt_argv = "uptickcoin-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // uptickcoin: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(uptickcoin);
    Q_INIT_RESOURCE(uptickcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse uptickcoin.conf
    ...
    // uptickcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
