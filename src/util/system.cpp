...
const char * const BITCOIN_CONF_FILENAME = "uptickcoin.conf";
 ...
 * https://github.com/uptickcoin/uptickcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //uptickcoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "uptickcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UpTickCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UpTickCoin
    // Mac: ~/Library/Application Support/UpTickCoin
    // Unix: ~/.uptickcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UpTickCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UpTickCoin";
#else
    // Unix
    return pathRet / ".uptickcoin";
#endif
#endif
}
