...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/uptickcoin-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    {
       nSubsidy = 90000000 * COIN; // 9,000,000,000 UTC (Pre-mine/Insta-mine)
    }
    else if(nHeight<=1052020)
    {
        nSubsidy = 21000 * COIN; // 2,366,820,000 UTC in 1051920 blocks
    }
    else if(nHeight<=3155860)
    {
        nSubsidy = 10500 * COIN; // 2,366,820,000 UTC in 2103840 blocks
    }
    else if(nHeight<=6311620)
    {
        nSubsidy = 5250 * COIN; // 1,775,115,000 UTC in 3155760 blocks
    }
    else if(nHeight<=9467380)
    {
        nSubsidy = 2625 * COIN; // 887,557,500 UTC in 3155760 blocks
    }
    else if(nHeight<=12623140)
    {
        nSubsidy = 1312.5 * COIN; // 443,778,750 UTC in 3155760 blocks
    }
    else if(nHeight<=15778900)
    {
        nSubsidy = 656.25 * COIN; // 221,889,375 UTC  in 3155760 blocks
    }
    else if(nHeight<=18934660)
    {
        nSubsidy = 328.125 * COIN; // 110,944,688 UTC  in 3155760 blocks
    }
    else if(nHeight<=22090420)
    {
        nSubsidy = 164.0625 * COIN; // 55,472,344 UTC  in 3155760 blocks
    }
    else
    {
        nSubsidy = 0 * COIN;
